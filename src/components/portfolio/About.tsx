...
import { User, GraduationCap, FolderGit2, Briefcase, MapPin, Clock, Github } from "lucide-react";
...
// Simple GitHub Graph placeholder
const GitHubGraph = () => (
  <div className="glass-card p-4">
    <div className="flex items-center gap-2 mb-3">
      <Github className="w-5 h-5 text-slate-400" />
      <span className="text-sm font-medium text-slate-300">GitHub Contributions</span>
    </div>
      ...
      {[...Array(35)].map((_, i) => (
        <div
          ...
          className={`w-3 h-3 rounded-sm ${
            Math.random() > 0.3 ? "bg-green-400" : "bg-slate-700"
          }`}
        />
      ))}
    </div>
...
export function About() {
  return (
    <section id="about" className="py-20 md:py-28 px-4 section-gradient-cyan relative overflow-hidden">
      ...
      <div className="absolute top-20 right-20 w-64 h-64 bg-gradient-to-br from-cyan-400/10 to-purple-400/10 rounded-full blur-3xl blob-morph" />

      <div className="mx-auto max-w-6xl relative z-10">
        <SectionReveal>
          <SectionHeading title="About Me" subtitle="From finance to full-stack engineering — driven by curiosity and continuous learning" number="01" />
          ...
          >
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-green-400/10 border border-green-400/30 text-green-400 text-sm font-medium">
              <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" />
              Available for Opportunities
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-slate-700/50 text-slate-300 text-sm">
              <MapPin className="w-4 h-4" />
              {personalInfo.location}
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-blue-400/10 border border-blue-400/30 text-blue-400 text-sm">
          ...
          <motion.div
            ...
            className="absolute top-32 right-8 glass-card px-4 py-2 text-sm text-cyan-400 border border-cyan-400/30"
          >
            🚀 Building the future
          </motion.div>

          <div className="grid md:grid-cols-3 gap-8 md:gap-12 items-start">
            {/* Avatar and GitHub Graph */}
